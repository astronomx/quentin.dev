...
const cellBase =
    "skill-cell flex aspect-square items-center justify-center border-background-muted p-4";
...
export default function SkillSection() {
    ...
    const grid = (
        <div className="grid w-fit max-w-full grid-cols-6">
            {leftCarouselSkills.map(({ name, Icon, large }, index) => (
                <div
                    key={name}
                    className={`${cellBase} skill-from-left col-span-2 border-b ${
                        index < leftCarouselSkills.length - 1 ? "border-r" : ""
                    }`}
                    title={name}
                >
            ...
            {rightCarouselSkills.map(({ name, Icon }) => (
                <div
                    key={name}
                    className={`${cellBase} skill-from-right border-r`}
                    title={name}
                >
                    <SkillIcon Icon={Icon} />
                </div>
